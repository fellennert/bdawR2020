...
In the beginning, the `readr` package will be introduced. It can be used for the import of the majority of files you will ever want to import and export. If the files become larger, `vroom` is a viable alternative. Beyond that, there exist some more packages for other types such as Excel or Stata files. I will provide you with a basic tutorial on how to deal with them, too. If you want to work with data in other programming environments, `feather` might be the way to go.
...
* `read_csv(file = "file.csv")` reads comma delimited files      
* `read_csv2(file = "file.csv")` reads semi-colon delimited files and treats commas as           
* `read_delim(file = "file.txt", delim = "|")` reads files which are delimited by whatever delimiter you specify (`|` in this case)        
* `read_fwf(file = "file.fwf", col_positions = c(1, 3, 5))` reads fixed width files. Here, some sort of data on the columns must be provided, e.g., their positions in the file       
* If the values are separated by whitespace, you can also use `read_tsv(file = "file.tsv")` or `read_table(file = "file.tsv")`
...
* If there are rows you do not want to be considered, you can use `skip =`. For instance, `read_csv("file.csv", skip = 6)` reads everything but the first six data rows (the very first row is normally used for column names and taken into consideration)       
...
* In some data sets, the first rows consists of comments that start with particular signs or special characters. Using `comment = ` allows you to skip these lines. For instance, `read_csv(file = "file.csv", comment = "#")` drops all the rows that beginn with a hash. 
...
When looking at the parsing failures here, what catches the eye is that the first 1000 values of `challenge$y` seem to be NA. Because `readr` only takes the first 1000 rows into account, it parses `challenge$y` as `logical`. However, it should be considered a `date` column. You can specify this using `col_type =`:
...
#                             col_types = cols(
#                               x = col_number(),
#  needs to be modified -->     y = col_double()
#                            ))
```
...
`.rds` files can be used to store singular R-specific objects (such as lists), `.RData` files can be used to store multiple R-specific objects. The former can be read in using `read_rds(file = "file.rds")`, the latter with `load("file.RData")`. More on `read_rds()` [here](https://readr.tidyverse.org/reference/read_rds.html) and `.RData` [here](http://www.sthda.com/english/wiki/saving-data-into-r-data-format-rds-and-rdata)
...
You can use `haven` for reading and writing SAS (suffixes `.sas7bdat`, `.sas7bcat`, and `.xpt`),  SPSS (suffixes `.sav` and `.por`), and STATA (suffix `.dta`) files.

The functions then are:   

* `read_sas(file = "file.sas7bdate"` and `write_sas(tibble, file = "file.sas7bdat")` for both `.sas7bdat` and `.sas7bcat` files. `read_xpt(file = "file.xpt")` reads `.xpt` files    
* `read_sav("file.sav")` and `read_por("file.por")` for `.sav` and `.por` files. `write_sav(tibble, file = "file.sav"` writes a the Tibble `tibble` to the file `file.sav`   
* `read_dta("file.dta")` and `write_dta(tibble, file = "file.dta")` read and write `.dta` files
...
`readxl` can be used to read Excel files. `read_excel(file = "file.xls")` works for both `.xls` and `.xlsx` files alike. It guesses the data type from the suffix. Excel files often consist of multiple sheets. `excel_sheets(file = "file.xlsx")` returns the name of the singular sheets. When dealing with an Excel file that contains multiple sheets, you need to specify the sheet you are after in the `read_excel()` function: `read_excel(file = "file.xlsx", sheet = "sheet_1")`. 
...
Download the "training files" from my iCloud drive (information on where I got the datasets from can be found in the "sources.md" file) <https://www.icloud.com/iclouddrive/0n5lNovQcjbKMp7uuEYhM5z6g#import_training_files>.

Read them in using the right functions. Specify the parameters properly. 
